...
//        if(xhr.responseJSON.code == 401){
        if(xhr.status == 401){
            parent.location.href = baseURL + 'manger';
        }
    }
});
...
//选择一条记录
function getSelectedRowParmId(id) {
    var grid = $("#"+id);
    var rowKey = grid.getGridParam("selrow");
    if(!rowKey){
    	alert("请选择一条记录");
...
//选择多条记录
function getSelectedRowsParmId(id) {
    var grid = $("#"+id);
    var rowKey = grid.getGridParam("selrow");
    if(!rowKey){
    	alert("请选择一条记录");
...
//弹出层
function show_window(width,height,url,title){
	 layer.open({
		  type: 2,
		  skin: 'layui-layer-molv',
		  title: title,
